...
class AlertListFactory:
    ...
    def get_alert_handler(self, too_input_file):
        ...
        elif 'reporters' in data.columns and 'creationdate' in data.columns and 'lastmodified' in data.columns:
            # TNS ToO format.
            return TNSAlertListXH(data)
        else:
...
class TNSAlertListXH(TooAlertList):
    """ToO input handling from the Transient Name Server (TNS), https://www.wis-tns.org/. Using Xander Hall's format.
    """
    def __init__(self, too_table):
        super().__init__(too_table)
    ...
    def generate_too_list(self):
        ...
        for entry in self.too_table:
            objid = entry['name']
            ra, dec = entry['ra'], entry['declination']
            date = entry['lastmodified']
            t = Time(date, format='iso', scale='utc')
            mjd = int(np.floor(t.mjd))
            instrument = entry['reporting_group']
            tooid = self.toodb.add_alert(objid, instrument, date, mjd, ra, dec)
            if tooid == 0:
                continue
            too_list.append(
                [ra, dec, 0., 0., 2000.0, 'XH/AP', 'FIBER', 'LO', 'BRIGHT', t.mjd, t.mjd + delta_t, tooid]
            )
